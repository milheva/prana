<x-public-layout>
    <x-slot name="title">Produk - Prana Medical</x-slot>
    ...
    <div class="bg-gradient-to-r from-primary to-secondary py-12 mb-8">
        <div class="container mx-auto px-4">
            ...
            <p class="text-white/90 mt-2 text-lg">Temukan alat kesehatan berkualitas untuk kebutuhan Anda</p>
        </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="card-body">
                    ...
                    <form method="GET" action="{{ route('products.index') }}">
                        ...
                        <div class="form-control mb-4">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Nama produk..." class="input input-bordered input-sm" />
                        </div>
                                ...
                                @foreach ($categories as $category)
                                    <label class="label cursor-pointer justify-start gap-2">
                                        <input type="radio" name="category" value="{{ $category->slug }}"
                                            ...
                                            {{ request('category') == $category->slug ? 'checked' : '' }} />
                                        <span class="label-text">{{ $category->name }}
                                            ({{ $category->products_count }})
                                        </span>
                                    </label>
                                @endforeach
                                <label class="label cursor-pointer justify-start gap-2">
                                    <input type="radio" name="category" value="" class="radio radio-sm"
                                        {{ !request('category') ? 'checked' : '' }} />
                        ...
                        <div class="form-control mb-4">
                            ...
                            <input type="number" name="min_price" value="{{ request('min_price') }}"
                                placeholder="Min" class="input input-bordered input-sm mb-2" />
                            <input type="number" name="max_price" value="{{ request('max_price') }}"
                                placeholder="Max" class="input input-bordered input-sm" />
                        </div>

                        <button type="submit" class="btn btn-primary btn-sm btn-block">Terapkan Filter</button>
                        <a href="{{ route('products.index') }}" class="btn btn-ghost btn-sm btn-block mt-2">Reset
                            Filter</a>
                    </form>
                </div>
            </div>
        ...
        <div class="flex-1">
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    <p class="text-sm text-base-content/70">Menampilkan {{ $products->count() }} dari
                ...
                <div class="dropdown dropdown-end">
                    ...
                    <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow">
                        <li><a
                                href="{{ route('products.index', array_merge(request()->all(), ['sort' => 'latest'])) }}">Terbaru</a>
                        </li>
                        <li><a
                                href="{{ route('products.index', array_merge(request()->all(), ['sort' => 'price_low'])) }}">Harga
                                Terendah</a></li>
                        <li><a
                                href="{{ route('products.index', array_merge(request()->all(), ['sort' => 'price_high'])) }}">Harga
                                Tertinggi</a></li>
                        <li><a
                                href="{{ route('products.index', array_merge(request()->all(), ['sort' => 'name'])) }}">Nama
                                A-Z</a></li>
                    </ul>
                </div>
            </div>
                        ...
                        <div class="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow">
                            <figure class="px-10 pt-10">
                                <div class="avatar placeholder">
                                    <div class="bg-neutral text-neutral-content w-full h-48 rounded-xl">
                                        <span class="text-3xl">{{ strtoupper(substr($product->name, 0, 2)) }}</span>
                                    </div>
                                </div>
                            </figure>
                            <div class="card-body">
                                ...
                                <div class="flex items-center gap-2 mt-2">
                                    @if ($product->isOnSale())
                                        <span class="text-xl font-bold text-error">Rp
                                            {{ number_format($product->discount_price, 0, ',', '.') }}</span>
                                        <span class="text-xs line-through text-base-content/50">Rp
                                ...
                                <div class="flex items-center gap-2 text-sm mt-2">
                                    @if ($product->inStock())
                                        <span class="badge badge-success badge-sm">Stok: {{ $product->stock }}</span>
                                    @else
            ...
            @else
                <div class="text-center py-16">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 mx-auto text-base-content/30 mb-4"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p class="text-xl text-base-content/70">Tidak ada produk ditemukan</p>
                    <a href="{{ route('products.index') }}" class="btn btn-primary btn-sm mt-4">Lihat Semua
                        Produk</a>
                </div>
            @endif
